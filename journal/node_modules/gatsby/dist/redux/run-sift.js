...
const applyFilters = (filterFields, firstOnly, nodeTypeNames, typedKeyValueIndexes, resolvedFields) => {
  const filter = filterFields ? getFilters(liftResolvedFields(toDottedFields(prepareQueryArgs(filterFields)), resolvedFields)) : [];
  let result;
...
const filterWithoutSift = (filter, nodeTypeNames, typedKeyValueIndexes) => {
  ...
  let lastPath = chainWithNeedle.pop(); // This can also be `$ne`, `$in` or any other grapqhl comparison op

  if (lastPath !== `$eq`) {
    return undefined;
  }

  return runFlatFilterWithoutSift(chainWithNeedle, targetValue, nodeTypeNames, typedKeyValueIndexes);
};
/**
...
const filterWithSift = (filter, firstOnly, nodeTypeNames, resolvedFields) => {
  ...
  return _runSiftOnNodes(nodes, filter, firstOnly, nodeTypeNames, resolvedFields, siftGetNode);
};
/**
